...
import { useInView } from "@/core/presentation/hooks/use-in-view";
import { LazyImage } from "@/core/presentation/components/shared/lazy-image";
...
export function DormCard({
  ...
  // Keep rootMargin large to start loading early
  ...
  // Mark loading state complete when first image loads
  ...
  const preloadImage = (index: number) => {
    if (sortedImages.length === 0 || imagesLoaded.value[index] === true) return;
    ...
    img.onerror = () => {
      // Mark as loaded even if error to prevent hanging
      imagesLoaded.value = { ...imagesLoaded.value, [index]: true };
    };
  ...
  const dormDetailsUrl = `/dorm/${dorm.id}`;
  ...
  return (
    <a 
      ...
      className="group relative block text-dormo-black"
    >
      <div
        ref={ref}
            ...
            <>
              {sortedImages.length > 0 ? (
                <AnimatePresence initial={false} custom={direction.value}>
                  ...
                  >
                    <LazyImage
                      src={sortedImages[currentImageIndex.value].url}
                      alt={`${dorm.name} - image ${currentImageIndex.value + 1}`}
                      className="object-cover h-full w-full transition-transform duration-300 group-hover:scale-105"
                      onLoaded={() => handleImageLoaded(currentImageIndex.value)}
                    />
                  </motion.div>
                </AnimatePresence>
              ) : (
              ...
              {!isLoading.value && (
                <>
                  ...
                  {/* Favorite Button - make sure it stops propagation */}
                  <button
                  ...
                  {/* Navigation Arrows - make sure they stop propagation */}
                  {sortedImages.length > 1 && isHovering.value && currentImageIndex.value > 0 && (
                    <button
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                  ...
                  {sortedImages.length > 1 && isHovering.value && currentImageIndex.value < sortedImages.length - 1 && (
                    <button
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                      ...
                      {sortedImages.slice(0, 5).map((_, index) => (
                        <button
                          ...
                          className={`w-1.5 h-1.5 rounded-full ${
                            ...
                            (index === 4 && currentImageIndex.value >= 5) ? "bg-white" : "bg-white/60"
                          }`}
        ...
        <div className="py-4 space-y-1">
          <div className="flex justify-between items-start">
            <div className="space-y-1 w-full">
              ...
              <p className="text-muted-foreground text-sm font-normal">{dorm.name}</p>
            </div>
          </div>
          <div className="flex justify-between items-end">
            <div>
              <p className="text-sm text-gray-700 font-normal">starts at</p>
